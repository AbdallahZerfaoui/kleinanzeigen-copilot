...
export function buildPrompt({ listing, profile, goalType, language = "de", selectedQuestions = [] }) {
  ...
  if (selectedQuestions && selectedQuestions.length > 0) {
    const qList = selectedQuestions.map(q => `- ${q}`).join("\n");
    questionsPart = language === "en"
      ? `\nIMPORTANT: Please politely include the following questions in the message:\n${qList}\n`
      : `\nWICHTIG: Bitte binde die folgenden Fragen höflich in die Nachricht ein:\n${qList}\n`;
  }
